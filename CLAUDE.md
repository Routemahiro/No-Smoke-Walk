...
- 2025-07-28 利用規約・プライバシーポリシーページを本番環境にデプロイ完了（デプロイ手順はdocs/DEPLOYMENT_PROCEDURES.mdを参照）
...
**❌ 発生したエラーと原因**
- **useSearchParams使用によるCSRフォールバック**: Next.js App RouterでuseSearchParamsを使用すると、Cloudflare Pagesのような静的ホスティングでは404エラーになる
- **Suspenseコンポーネントによるローディング固定**: Suspenseでラップしたコンポーネントが初期ローディング状態で静的生成され、実際のコンテンツが表示されない
- **useEffectによる動的データフェッチ**: 初期レンダリング時にローディング状態になり、静的HTMLにコンテンツが含まれない
- **デプロイディレクトリの間違い**: プロジェクトルート（.）ではなく、ビルド成果物（out）をデプロイする必要がある

**✅ 対策と解決策**
- **useState + propsで状態管理**: useSearchParamsの代わりにReact useStateでフィルタ状態を管理
- **静的データの直接設定**: useEffectでのデータフェッチを避け、モジュールレベルで静的データを定義
- **Suspense削除**: 不要なSuspenseコンポーネントを削除し、直接レンダリング
- **正しいデプロイパス**: `npx wrangler pages deploy out` でビルド成果物をデプロイ

### 開発時のチェックポイント

**静的サイト生成での必須確認事項**
1. **ビルドログ確認**: `○ (Static)` マークが付いているかチェック
2. **生成HTMLの内容確認**: `out/` ディレクトリのHTMLに実際のコンテンツが含まれているか
3. **クライアントサイド機能の制限**: useSearchParams, useRouter等のNext.js フック使用時は静的生成対応の実装が必要
4. **Cloudflare Pages制約**: CSR（Client-Side Rendering）は動作しないため、完全静的生成必須

### 今後の開発指針

**Cloudflare Pages + Next.js開発時のルール**
- フィルタ機能は `useState` + props でクライアントサイド実装
- 動的データは静的定義またはAPI分離
- ビルド前に必ず静的生成の確認
- デプロイ前に `out/` ディレクトリの内容確認